...
import { getShoppingCart, selectShoppingCarts } from "@/stores/ShoppingCarts/slice";
...
function sumCartAmounts(cartDetails:any) {
  return cartDetails.items.reduce((total:number, item:any) => total + item.amount, 0);
}
...
function Jwt(props: any) {
  ...
  return(
    <div className="w-14 h-14 bg-primary/20  rounded-full px-2 py-2 border border-primary hover:bg-purple-100">
      {
          JWT && <a target="_blank" href={`https://pagos.miniswimmer.cl/${JWT}`}>
          <Lucide icon="ShoppingCart" className="w-10 h-10 p-2 text-slate-800" />{" "}
        </a>
      }
    </div>
  )
      ...
      <div className="overflow-auto xl:overflow-visible">
        <Table className="border-b border-slate-200/60">
          ...
          <Table.Tbody>
            ...
            {Array.isArray(shoppingCarts) && shoppingCarts.map((item:any, index) => {
             const totalAmount = item?.cartDetails && sumCartAmounts(item?.cartDetails);
            ...
            const formattedDate: string = date.toLocaleDateString('es-CL', options);
              ...
              <Table.Tr key={index} className="[&_td]:last:border-b-0">
                ...
                <Table.Td className=" py-4 border-dashed">
                  <div className="flex items-center justify-center flex-col">
                    <p className="uppercase font-thin text-sm text-center">$ {formatCurrency(totalAmount)}</p>
                  </div>                   
                </Table.Td>
                <Table.Td className=" py-4 border-dashed">
                ...
                <Table.Td className=" border-dashed w-1">
                  <Button
                    ...
                    >
                      <Lucide icon="AlignLeft" className="w-10 h-10 p-2 text-slate-400" />{" "}
                  </Button>
                </Table.Td>
                <Table.Td className="border-dashed w-1">
                  
                  { item?.status !== "AUTHORIZED" && <Jwt cartId={item?.id}/>}
                {
                // item?.status !== "AUTHORIZED" && JWT &&
...
function Main() {
  ...
  dispatch(setBreadcrumb({first:"Carros de compra", firstURL:"transactions"}));

  
  
  useEffect(() => { 
    (async () => await dispatch(getShoppingCart({})) )(); 
